...
class ModelNet40Cls(data.Dataset):
    def __init__(self, num_points, folder, base_dir, aug_transforms=None, train=True, download=False):
        super().__init__()
...
if __name__ == "__main__":
    from Modelnet_data import data_utils as d_utils
    ...
    dset = ModelNet40Cls(1024, 'modelnet40_ply_hdf5_2048', '/home/segoln/data/', aug_transforms=None, train=True)
    print(dset[0][0])
    print(dset[0][1])
    print(len(dset))
    dloader = torch.utils.data.DataLoader(dset, batch_size=32, shuffle=True)
    j, data = next(enumerate(dloader, 0))
    points, targets = data
    tdset = ModelNet40Cls(1024,'modelnet40_ply_hdf5_2048', '/home/segoln/data/',
                          transforms=a_transforms, train=False)
    print("sdf")
