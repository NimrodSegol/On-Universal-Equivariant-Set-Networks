...
from models import *
from datasets import modelnet_regression_dataset
...
parser.add_argument('--model', type=str, default='GraphNet',  help='model')
...
dataset = modelnet_regression_dataset(opt.path, num_points=512, train=True, graph=gr)
dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize,
                                         shuffle=True, num_workers=int(opt.workers))


test_dataset = modelnet_regression_dataset(opt.path, num_points=512, train=False, graph=gr)
...
for width in np.linspace(256*3, 5,  15).astype(np.int):
    ...
    folder_name = os.path.join('modelnet_regression_log', opt.model, 'width' + str(width))
